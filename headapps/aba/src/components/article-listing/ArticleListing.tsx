        ...
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
              ...
              {titleOptional && (
                <div className="@md:mb-0 mb-4">
                  <Text
                    ...
                    id={sectionId}
                    field={titleOptional}
                    className="font-heading @md:text-6xl text-heading text-4xl font-normal leading-[1.20] tracking-tighter"
                  />
          ...
          <div className="@md:grid-cols-2 mb-[28px] grid gap-8">
            {featuredArticles.map((article, index) => (
              <article key={index} className="@md:mb-0 group/article mb-6">
                {isPageEditing ? (
                  <div className="rounded-xl shadow-[0_0_5px_0_rgba(0,0,0,0.1)] @md:mb-0 relative mb-4 aspect-[3/2] w-full overflow-hidden border border-border">
                    <Image
                      src={article.image}
                      alt={article.title}
                      fill
                  ...
                  <div
                    className="rounded-xl shadow-[0_0_5px_0_rgba(0,0,0,0.1)] @md:mb-0 relative mb-4 aspect-[3/2] w-full cursor-pointer overflow-hidden border border-border"
                ...
                <div className="@md:p-8">
                  {isPageEditing ? (
                    <h3 className="font-heading text-heading text-3xl font-medium leading-[1.30] -tracking-[0.9px]">
                      {article.title}
                    </h3>
                  ) : (
                    <Link field={{ value: { href: article.link } }} className="block">
                      <h3 className="font-heading text-heading text-3xl font-medium leading-[1.30] -tracking-[0.9px] decoration-1 underline-offset-4 group-hover/article:text-primary group-hover/article:underline group-focus/article:underline">
                        {article.title}
                      </h3>
                    </Link>
                  )}
                  <p className="text-muted-foreground text-normal mt-4 line-clamp-2 text-lg leading-[1.5] tracking-tighter">
                    {article.summary}
                  </p>
          ...
          <div className="@sm:grid-cols-2 @lg:grid-cols-3 grid gap-8">
            {regularArticles.map((article, index) => (
              <article
                key={index}
                className="@md:p-8 rounded-xl border border-transparent shadow-sm hover:border-border hover:bg-surface-subtle focus:ring-ring group/article flex h-full flex-col p-4 transition-colors focus:outline-none focus:ring-2"
              >
                <div>
                  {isPageEditing ? (
                    <h3 className="font-heading text-heading text-2xl font-medium leading-normal tracking-tighter">
                      {article.title}
                    </h3>
                  ) : (
                    <Link field={{ value: { href: article.link } }} className="block">
                      <h3 className="font-heading text-heading text-2xl font-medium leading-normal tracking-tighter decoration-1 underline-offset-4 group-hover/article:text-primary group-hover/article:underline group-focus/article:underline">
                        {article.title}
                      </h3>
                    </Link>
                  )}
