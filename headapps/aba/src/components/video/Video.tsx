...
export const VideoBase: React.FC<VideoComponentFields> = (props) => {
  ...
  return !videoUrl ? (
    <p className="bg-muted text-muted-foreground flex aspect-video items-center justify-center rounded-xl border border-border">
      <span className="font-body font-semibold">Please add video</span>
    </p>
  ) : (
    <motion.div
      whileHover="hover"
      initial="initial"
      className="max-w-screens-2xl relative z-10 mx-auto overflow-hidden rounded-xl shadow-[0_0_5px_0_rgba(0,0,0,0.1)]"
    >
          ...
          {displayIcon && (
            <div className="absolute inset-0 flex max-w-[30%] items-center justify-center">
              <svg
                ...
                className="text-primary absolute bottom-0 left-0 block h-auto w-full"
                aria-hidden="true"
              >
                <path
