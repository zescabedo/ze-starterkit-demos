        ...
        <div className=" @md:py-16 @lg:py-20 @lg:grid-cols-[320px,1fr] @lg:gap-12 @xl:gap-16 mx-auto grid max-w-screen-xl gap-8 py-10">
          <div className="@lg:pr-0 space-y-4 px-6">
            {heading?.jsonValue && (
              <Text
                tag="h2"
                className="font-heading text-heading @lg:text-7xl -ml-1 text-pretty text-5xl font-normal leading-[1.25] tracking-tighter"
                field={heading?.jsonValue}
              />
            )}
            {description?.jsonValue && (
              <Text
                className="font-body text-base font-normal text-muted-foreground"
                tag="p"
