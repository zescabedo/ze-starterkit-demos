...
test('GET /ai/markdown/Products/Aero returns 200 and markdown content', async () => {
    const resp = await axios.get(`${BASE_URL}/ai/markdown/Products/Aero`, { validateStatus: () => true, timeout: 10000 });
    expect(resp.status).toBe(200);
    expect(resp.headers['content-type']).toContain('text/markdown');
    expect(typeof resp.data).toBe('string');
    expect(resp.data.length).toBeGreaterThan(0);
